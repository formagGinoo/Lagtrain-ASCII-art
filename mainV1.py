...
frame_interval = 0.0659
# frame_interval = 1 / 12
frame_size = 200


def play_audio():
    playsound.playsound("song.mp3", block=False)


def play_video():
    for frame_number in range(1, 3776):
        start_time = time.time()
        file_name = r"TextFiles/" + "aot_" + str(frame_number) + ".txt"
...
def extract_frames(video_path):
    ...
    while current_frame < total_frames:
        ...
        frame_name = r"ExtractedFrames/" + "song_" + str(current_frame) + ".jpg"
...
def pixels_to_ascii(image_frame):
    ...
    characters = "".join([ASCII_CHARS[pixel // 75] for pixel in pixels])
    return characters
...
def ascii_generator(image_frame, index):
    ...
    file_name = r"TextFiles/" + "aot_" + str(index) + ".txt"
...
def check_txt():
    ...
    verified_frames = 1
    for frame_count in range(1, 3776):
        path_to_file = r'TextFiles/' + 'aot_' + str(frame_count) + '.txt'
        if os.path.isfile(path_to_file):
            verified_frames += 1
    if verified_frames > 3000:
